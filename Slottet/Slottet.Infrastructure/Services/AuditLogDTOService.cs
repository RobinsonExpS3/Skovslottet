...
namespace Slottet.Infrastructure.Services {
    public class AuditLogDTOService : IAuditLogDTOService {
        ...
        public async Task<IEnumerable<AuditLogDTO>> GetAllAuditLogsAsync() {
            ...
            await EnrichAuditLogsAsync(auditLogs);
            return auditLogs;
        }
        ...
        private async Task EnrichAuditLogsAsync(List<AuditLogDTO> auditLogs) {
        ...
        private async Task<string> ResolveFallbackSubjectAsync(IReadOnlyDictionary<string, string?> keyValues) {
            ...
            foreach (var keyValue in keyValues) {
                resolvedParts.Add(await ResolveValueAsync(keyValue.Key, keyValue.Value) ?? "Ikke givet");
            }
        ...
        private static string JoinSubjectParts(params string?[] parts) {
            ...
            return string.IsNullOrWhiteSpace(subject) ? "Ikke givet" : subject;
        }
